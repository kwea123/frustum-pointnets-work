...
from frustum_pointnets_v2 import get_3d_box_estimation_v2_net
...
def get_instance_seg_v1_net_modified(point_cloud, one_hot_vec,
    ...
    c1 = tf_util.conv2d(net, 128, [1,1],
                         ...
                         bn=True, is_training=is_training,
                         scope='conv1', bn_decay=bn_decay)
    c1_mp = tf_util.max_pool2d(c1, [num_point,1], padding='VALID', scope='mp1')
    ...
    c2 = tf_util.conv2d(c1, 128, [1,1],
                         ...
                         bn=True, is_training=is_training,
                         scope='conv2', bn_decay=bn_decay)
    c2_mp = tf_util.max_pool2d(c2, [num_point,1], padding='VALID', scope='mp2')
    ...
    c3 = tf_util.conv2d(c2, 256, [1,1],
                         ...
                         bn=True, is_training=is_training,
                         scope='conv3', bn_decay=bn_decay)
    c3_mp = tf_util.max_pool2d(c3, [num_point,1], padding='VALID', scope='mp3')
    ...
    c4 = tf_util.conv2d(c3, 512, [1,1],
                         ...
                         bn=True, is_training=is_training,
                         scope='conv4', bn_decay=bn_decay)
    c4_mp = tf_util.max_pool2d(c4, [num_point,1], padding='VALID', scope='mp4')
    c4_mp = tf.tile(c4_mp, [1, num_point, 1, 1])
    c4 = tf.concat(axis=3, values=[c4, c4_mp])
    c5 = tf_util.conv2d(c4, 512, [1,1],
                         padding='VALID', stride=[1,1],
                         bn=True, is_training=is_training,
                         scope='conv5', bn_decay=bn_decay)
    ...
    d1 = tf_util.conv2d(global_feat_expand, 512, [1,1],
                         ...
                         bn=True, is_training=is_training,
                         scope='conv6', bn_decay=bn_decay)
    d1_mp = tf_util.max_pool2d(d1, [num_point,1], padding='VALID', scope='mp6')
    d1_mp = tf.tile(d1_mp, [1, num_point, 1, 1])
    d1 = tf.concat(axis=3, values=[d1, d1_mp])
    d2 = tf_util.conv2d(d1, 256, [1,1],
                         padding='VALID', stride=[1,1],
                         bn=True, is_training=is_training,
                         scope='conv7', bn_decay=bn_decay)
    d2_mp = tf_util.max_pool2d(d2, [num_point,1], padding='VALID', scope='mp7')
    d2_mp = tf.tile(d2_mp, [1, num_point, 1, 1])
    d2 = tf.concat(axis=3, values=[d2, d2_mp])
    d3 = tf_util.conv2d(d2, 128, [1,1],
                         padding='VALID', stride=[1,1],
                         bn=True, is_training=is_training,
                         scope='conv8', bn_decay=bn_decay)
    d3_mp = tf_util.max_pool2d(d3, [num_point,1], padding='VALID', scope='mp8')
    d3_mp = tf.tile(d3_mp, [1, num_point, 1, 1])
    d3 = tf.concat(axis=3, values=[d3, d3_mp])
...
def get_model(point_cloud, one_hot_vec, is_training, bn_decay=None):
    ...
    logits, end_points = get_instance_seg_v1_net_modified(\
